...
function evaluateAddInput(tableRow) {
  if (
    parseInt(tableRow.children[4].value) >
    parseInt(tableRow.children[3].innerText)
  ) {
    tableRow.children[4].value = '';
    alert('La cantidad que escribiste supera a la disponible');
    return false;
  } else if (
    isNaN(parseInt(tableRow.children[4].value)) ||
    isNaN(Number(tableRow.children[4].value))
  ) {
    tableRow.children[4].value = '';
...
function evaluateRemoveInput(tableRow) {
  if (
    parseInt(tableRow.children[3].value) >
    parseInt(tableRow.children[2].innerText)
  ) {
    tableRow.children[3].value = '';
    alert('La cantidad que escribiste supera a la disponible');
    return false;
  } else if (
    isNaN(parseInt(tableRow.children[3].value)) ||
    isNaN(Number(tableRow.children[3].value))
  ) {
    tableRow.children[3].value = '';
    alert('La cantidad que escribiste debe ser un número entero');
    return false;
...
function fillTable(table, productsArray, tableType) {
  if (productsArray.length > 0) {
    ...
    let filledTableHeader = fillHeader(
      tableStructure[0],
      productsArray,
      tableType
    );
